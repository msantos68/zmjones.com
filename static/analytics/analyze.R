...
paste(length(unique(df$remote_ip[df$key == "static/papers/eeesr_manuscript.pdf"])), "eeesr views")
...
p <- p + theme_bw()
ggsave("daily_views.png", plot = p, width = 10, height = 5)
...
write.csv(views_by_key, "views_by_key.csv", row.names = FALSE)
...
p <- p + coord_flip()
p <- p + theme_bw()
ggsave("top_keys.png", plot = p, width = 6, height = 8)
